...
const client =
  g.tkMongoClient ?? new MongoClient(uri);

if (!g.tkMongoClient) {
  g.tkMongoClient = client;
}

const clientPromise =
  g.tkMongoPromise ?? client.connect();

if (!g.tkMongoPromise) {
  g.tkMongoPromise = clientPromise;
}
  ...
  if (!shop) {
    shop = await Shop.findOneAndUpdate(
      ...
      },
      {
        $setOnInsert: {
          userId: user._id,
          shopName: `${name || email.split('@')[0]}'s Shop`,
          currency: 'BDT / ৳',
        },
      },
      {
      ...
      if (user?.id) {
        ...
        const ensured =
          await ensureShop(
            String(token.id),
            String(
              token.name || 'Shop Owner'
            ),
            String(
              token.email || 'shop@example.com'
            ),
