...
export const TransactionInputSchema = z
  .object({
    ...
    paidAmount: z.coerce.number().finite().min(0).optional(),
  ...
  .superRefine((v, ctx) => {
    if (
      ...
      (!v.partyId || v.amount <= 0)
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['partyId'],
        message: 'Party and positive amount are required',
      });
    }

    if (
      ['STOCK_IN', 'STOCK_OUT'].includes(v.type) &&
      (!v.productId || v.quantity <= 0)
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['productId'],
        message: 'Product and positive quantity are required',
      });
    }

    if (v.type === 'EXPENSE' && v.amount <= 0) {
      ctx.addIssue({
        code: 'custom',
        path: ['amount'],
    ...
    if (
      v.type === 'PAYMENT_TO_SUPPLIER' &&
      v.amount <= 0
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['amount'],
        message: 'Supplier payment amount must be positive',
      });
    }
  });
...
export type PaymentInput = z.infer<
  typeof PaymentInputSchema
>;

export const ClosingInputSchema = z.object({
